...
feature <- function(LR_dir, HR_dir, n_points=1000){
  ...
  output <- function(ori_row, ori_col){
    ...
    response <- c(highdata[2*row-1,2*column-1,],
                  highdata[2*row-1,2*column,],
                  highdata[2*row,2*column-1,],
                  highdata[2*row,2*column,])
    dif <- response - center
    HR_featmatrix<- array(0, c(4,3))
    HR_featmatrix[, 1]<- dif[c(1,4,7,10)]
    HR_featmatrix[, 2]<- dif[c(2,5,8,11)]
    HR_featmatrix[, 3]<- dif[c(3,6,9,12)]
    return(HR_featmatrix)
  }
